...
function Portfolio(){
	...
	this.win = $(window);
	this.win.bind('resize.Portfolio', function(e){ that.resizeHandler() });
	this.resizeHandler();
...
Portfolio.prototype.createStructure = function(){
	...
	this.structImg.bind('dragstart', function(e){e.preventDefault();});
...
Portfolio.prototype.fillItemsArray = function(listItems){
	...
	for (var i = 0; i < list.length; i++){
		var parent = list.eq(i);
		var imageObj = $('> figure img', parent);
		imageObj.bind('dragstart', function(e){e.preventDefault();});
...
Portfolio.prototype.itemClickHandler = function(clickedObject, objectIndex){
	...
	if ( this.ci.obj == null && this.ci.ind == null ){
		...
		this.openItem(obj, id); //Only opens the clicked item
		
	}
	else if ( id == this.ci.ind ) {
		return false;
		...
		obj.addClass('active');
		this.createScrollbar(obj);
		this.closeItem(obj, id, true); //Closes the item which was open and opens the clicked one
	}
...
Portfolio.prototype.checkFinalPosition = function(leftValue, maxLeftValue, objectToAnimate, invertedDirection){
	...
	if (inv){
		if (val >= 0) { obj.animate({ left: 0 }, 700); }
		else if (Math.abs(val) >= maxVal) { obj.animate({ left: -maxVal }, 700); }
	}
	else {
		if (val <= 0) { obj.animate({ left: 0 }, 700); }
		else if (val >= maxVal) { obj.animate({ left: maxVal }, 700); }
	}
...
function Navigation(){
	...
	this.scrollable = $('html, body');	
	this.buttons.bind('click.Navigation', function(e){ e.preventDefault(); that.goToItem($(this)) } );
	this.totalH = $('body').height();
	this.win = $(window);
	this.win.bind('scroll.Navigation', function(e){ that.scrollHandler() });
...
ProfileGallery.prototype.startTimeout = function(){
	...
	var that = this;
	this.timer = setTimeout(function(){ that.changeImage() }, 4000);
